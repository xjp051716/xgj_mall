...
export default {
  ...
  methods:{
    ...
    saveAddr(){
      let is_default = this.isdefault == false ? 0 : 1 ;
      let isdefault = 0;
      this.axios.post('defaultAddr',this.qs.stringify({'is_default':isdefault})).then(res=>{
        let data = this.qs.stringify({"receiver":this.receiver.trim(),"cellphone":this.cellphone.trim(),"province":this.province.trim(),"city":this.city.trim(),"county":this.county.trim(),"address":this.fullAddress.trim(),"is_default":is_default,"tag":this.tag,"aid":this.aid});
