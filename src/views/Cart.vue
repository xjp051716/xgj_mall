  ...
  <div id="scrollDiv" class="rela">
    ...
    <div class="cart">
      <div class="cart_title">
        ...
        <div @click="manage">
          {{
            ismanage == true ? (manage_font = "完成") : (manage_font = "管理")
          }}
        </div>
      </div>
      <!-- 购物车标题 end-->
    ...
    <div class="cart_title_replace" v-show="isShow">
      ...
      <span class="cart_title_replace_font"
        >购物车({{ shoppingList.length }})</span
      >
      <span @click="manage">{{
        ismanage == true ? (manage_font = "完成") : (manage_font = "管理")
      }}</span>
    ...
    <!-- 购物车主体-->
    <div class="shoppingMain" v-show="!allSelectDis">
      <div
        class="shoppingItem"
    ...
    <!-- 购物车为空时购物车界面 -->
    <div class="noGoods" v-show="allSelectDis">
      <!-- logo -->
      <div>
        <img class="nogoods_logo" src="../assets/cart_img/noGoods.png" alt="" />
      </div>
      <!-- 文字 -->
      <div class="nogoods_font1">购物车竟然是空的</div>
      <div class="nogoods_font2">是我家的零食不好吃嘛?呜呜呜- -</div>
      <!-- 跳转首页链接 -->
      <div class="router_index" @click="router_index">去逛逛</div>
    </div>
    <div class="guess_container">
      <div class="guess_title">
        <img src="../assets/cart_img/guess_title_hert.png" alt="" />
        猜你喜欢
      </div>
      <div class="guessYouLike">
        <div
          class="guess_item"
          v-for="(item, index) in products[0]"
          :key="index"
          @click="goDetails(item.laptop_id)"
        >
          <img :src="item.pic" alt="" />
          <div class="guess_item_title">{{ item.title }}</div>
          <div class="guess_item_price">￥{{ item.price }}</div>
        </div>
      </div>
    </div>
    <div class="bottomMain">
      ...
      <div class="little_flex_left">
        <div
          class="selectCircle"
          :class="{ checked: allSelect, allSelectDis: allSelectDis }"
...
export default {
  ...
  methods: {
    ...
    allGoodsSelect() {
      ...
      if (this.allSelect == true) {
        ...
        for (var i = 0; i < this.shoppingList.length; i++) {
          ...
          this.sum =
            this.sum +
            this.shoppingList[i].pre_price * this.shoppingList[i].count;
          // 结算商品的数量
          this.manageNum = this.manageNum + 1;
          // selectgoods数组全设为1
    ...
    buyGoods() {
      ...
      var order_number =
        Date.parse(new Date()) +
        "" +
        Math.ceil(Math.random().toFixed(4) * 10000);
      ...
      shoppingList.forEach((item, index) => {
        if (item.isSelect == true) {
          ...
          this.axios.post(
            "/postOrderdetail",
            this.qs.stringify(order_detail_obj)
          );
        }
  ...
  mounted() {
    ...
    // 获取猜你喜欢数据
    this.axios.get("/getProducts?category=2").then((res) => {
