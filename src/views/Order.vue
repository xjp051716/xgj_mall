    ...
    <mt-tab-container v-model="selectTab" class="orderList" v-else>
      <mt-tab-container-item id="1">
        <div v-for="(item,index) in orderList" :key="index" class="orderItem">
          <div @click="toDetail(item.order_id)" class="order-to">  
            <div class="order-pic">
              <img :src="item.pic">
            </div>
          ...
          <div class="order-btn-box">
            <button class="order-btn pay-btn" v-if="item.status==1" @click="pay(item.order_id)">付款</button>
...
export default {
  ...
  methods:{
    ...
    pay(id){
      this.$router.push('/orderdetails?order_id='+id)
    }
  }
}
</script>
