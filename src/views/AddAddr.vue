...
export default {
  ...
  methods:{
    checkReceiver(){
      var RegExp=/^[\u4e00-\u9fa5]{2,6}$/;
      if(RegExp.test(this.receiver.trim())==true){
        return true;
      }else{
        return false;
      }
    ...
    checkPhone(){
      ...
      if(RegExp.test(this.cellphone.trim())==true){
        return true;
      }else{
        return false;
      }
    ...
    checkFullAddr(){
      var RegExp=/^[\u4e00-\u9fa5\w]+$/;
      if(RegExp.test(this.fullAddress.trim())==true){
        return true;
      }else{
        return false;
      }
    ...
    saveAddr(){
      ...
      var isdefault = 0;
      this.axios.post('defaultAddr',this.qs.stringify({'is_default':isdefault})).then(res=>{
        var data = this.qs.stringify({"receiver":this.receiver.trim(),"cellphone":this.cellphone.trim(),"province":this.province.trim(),"city":this.city.trim(),"county":this.county.trim(),"address":this.fullAddress.trim(),"is_default":is_default,"tag":this.tag,"user_id":this.uid});
        !this.checkReceiver() ? this.$messagebox.alert('收件人2~6位中文字符') : !this.checkPhone() ? this.$messagebox.alert('请填写正确的手机号码') : !this.check(this.province) ? this.$messagebox.alert('请填写所在省份') :  !this.check(this.city) ? this.$messagebox.alert('请填写所在城市') : !this.check(this.county) ? this.$messagebox.alert('请填写所在地区') : !this.checkFullAddr() ? this.$messagebox.alert('请填写详细地址') :
